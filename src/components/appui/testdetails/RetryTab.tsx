...
export function RetryTab({
    ...
    const sortedAttempts = [...attempts].sort((a, b) => a.key.localeCompare(b.key));
    ...
    const renderAttemptSummary = (test: TestResultItem, index: number) => (
        <div
            ...
            className={cn(
                ...
                test.key === currentKey ? "border-primary bg-primary/5" : "border-border bg-card/50",
                compareIndices.includes(index) && "ring-2 ring-primary/40 ring-offset-1"
            )}
            onClick={() => {
                // Toggle selection for comparison
                if (compareIndices.includes(index)) {
                    if (compareIndices[0] === index) setCompareIndices([compareIndices[1], index]);
        ...
        >
            ...
            {test.key === currentKey && (
                <Badge variant="outline" className="mt-2 text-[8px] h-4 py-0 leading-none">
                    Viewing
                </Badge>
            )}
        </div>
    ...
    const ComparisonColumn = ({ test, title }: { test: TestResultItem, title: string }) => (
        <div className="flex-1 min-w-0 space-y-4">
            ...
            <div className="space-y-2">
                ...
                <div className="bg-muted/30 rounded p-2 text-[10px] font-mono h-32 overflow-y-auto whitespace-pre-wrap">
                    {test.logs || "No logs available for this attempt."}
                </div>
            </div>
        </div>
    );
    ...
    return (
        <TabsContent value="retries" className="space-y-6 pt-4">
            ...
            <div className="flex flex-col md:flex-row gap-6">
                <ComparisonColumn test={leftAttempt} title={`Attempt ${compareIndices[0] + 1}`} />
                <div className="hidden md:block w-px bg-border shrink-0" />
                <ComparisonColumn test={rightAttempt} title={`Attempt ${compareIndices[1] + 1}`} />
            </div>
        </TabsContent>
    );
}
